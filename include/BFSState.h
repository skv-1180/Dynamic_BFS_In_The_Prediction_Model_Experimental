...
class BFSState
{
   public:
    ...
    std::vector<std::unordered_set<int>> UP;      // upper-parents UP[v]
    std::vector<std::unordered_set<int>> outAdj;  // out-neighbours Out[v]
    std::vector<std::unordered_set<int>> inAdj;   // in-neighbours  In[v]
    AlgorithmMode mode{AlgorithmMode::FULLY_DYNAMIC};                          // algorithm mode
   public:
